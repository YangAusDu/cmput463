...
for i = 1:size(E, 1),
    ...
    for j = 1:length(F),
        ...
        if (~isempty(indx)),
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ind_arr = IndexToAssignment(1:length(F(j).val),F(j).card);
            zero_arr = find(ind_arr(:,indx) ~= x);
            zero_indx = IndexToAssignment(zero_arr.', F(j).card);
            F(j) = SetValueOfAssignment(F(j), zero_indx, 0) ;
            %disp(F(j).val);            
            %disp(index_arr)
